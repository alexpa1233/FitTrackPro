...
function ExerciseEditCustom() {
  const { id } = useParams(); // id del ejercicio a editar
  ...
  const fetchExercise = async () => {
    try {
      ...
      setSelectedType(exerciseData.type_id);
      setImage(exerciseData.image);
      setPreview(exerciseData.image); // Suponemos que la URL de la imagen viene en image
      // También puedes cargar los types desde la API si es necesario
      fetchTypes();
    } catch (error) {
      Swal.fire("Error", "Failed to fetch exercise", "error");
    }
  };
  ...
  const handleUpdate = async (e) => {
    ...
    formData.append("user_id", getUser().id); // Asegúrate de obtener el usuario autenticado
...
export default ExerciseEditCustom;
